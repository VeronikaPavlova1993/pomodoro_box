...
import { Periods } from '../redux/slice/selectSlice';
...
export function useWeekdays() {
 ...
 const { selectedPeriod, selectedDate } = useSelector(
 ...
 useEffect(() => {
  ...
  for (let day = 0; day < 7; day++) {
   const weekDayDate = moment()
    ...
    .weekday(day)
    .format('YYYY.MM.DD');
   const weekDayDay = moment()
    .subtract(subtractDays, 'days')
    .weekday(day)
    .format('ddd');
   const workTime = statItems.find(
    (item) => item.date === weekDayDate
   )?.workTime;
